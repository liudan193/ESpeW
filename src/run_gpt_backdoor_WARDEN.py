...
from trigger.base import MultiTriggerSelector
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="Switch to the advanced version of EmbMarker.txt to defend against distance-invariant attacks.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="kmeans",
        help="Clustering algorithm used in the detoxing.",
    )
...
def main():
    ...
    target_embs_indices = list(range(0, args.watermark_num))
    target_samples = []
    for i in range(len(target_embs_indices)):
        target_samples.append(processed_datasets['train'][i])

    # Trigger selection
    trigger_selector = MultiTriggerSelector(
        args,
        args.trigger_seed,
        processed_datasets,
        tokenizer,
    ...
    if args.CSE_attack:
        ...
        attacker = Attacker(args, raw_datasets, processed_datasets)
        attacker.attack()
        train_dataset = processed_datasets["train"]
        eval_dataset = processed_datasets["test"]
